...
class ERPNextCRMSettings(Document):
	...
	def validate_if_erpnext_installed(self):
		if not self.is_erpnext_in_different_site:
			if "erpnext" not in frappe.get_installed_apps():
				frappe.throw(_("AiBizzApp ERP is not installed in the current site"))
	...
	def create_custom_fields_in_remote_site(self):
		...
		try:
			...
			frappe.throw("Error while creating custom field in AiBizzApp ERP, check error log for more details")
...
def get_customer_link(crm_deal):
	...
	if not erpnext_crm_settings.is_erpnext_in_different_site:
		...
		try:
			...
			frappe.throw(_("Error while fetching customer in AiBizzApp ERP, check error log for more details"))
...
def create_prospect_in_remote_site(crm_deal, erpnext_crm_settings):
	try:
		...
		frappe.throw(_("Error while creating prospect in AiBizzApp ERP, check error log for more details"))
...
def create_customer_in_remote_site(customer, erpnext_crm_settings):
	...
	try:
		...
		frappe.throw(_("Error while creating customer in AiBizzApp ERP, check error log for more details"))
